...
TESTING_PV=${TESTING_PV:-'2.2.01.20271'}
...
./bootstrap-prefix.sh "${EPREFIX}" latest_tree # Support Mac OS X 10.7 and ppc64-linux
...
./bootstrap-prefix.sh "${EPREFIX_TMP}" zlib126 # Fix failure on ppc64-linux
...
echo "=app-editors/nano-2.3.1" >> "${EPREFIX}/etc/portage/package.mask"
echo "=app-editors/nano-2.3.1-r1" >> "${EPREFIX}/etc/portage/package.mask"
...
emerge --oneshot '<sys-libs/zlib-1.2.5-r3' #Workaround bug #407211
